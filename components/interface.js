...
export default function Interface(){
    ...
    return(
        ...
        >
            <div className="field">
                <label className="label is-size-4" htmlFor="url">Tweet URL: </label>
            ...
            <div className="field is-grouped is-grouped-multiline is-grouped-centered">
                {
                    Object.entries(options).map((k,v)=><Checkbox key={v} name={k[0]} label={k[1]} onChange={handleInputChange}/>)
                }
            </div>
            ...
            <div className="field">
                ...
                <h4 className="is-size-4 has-text-danger" style={{display: (error==="") ? "none":"block"}}>{error}</h4>
        </form>
        <PuffLoader color={"#1da1f2"} size={windowSize.width*.2} loading={tweetLoading} css={{margin: "auto"}}/>
        <div style={{display:tweetLoaded?"block":"none"}}>
            <figure className={`image p-3 ${styles.tweetFigure}`}>
                    {
                        (tweetLoaded)  &&
                        <Image src={tweetImage.url} width={tweetImage.width} height={tweetImage.height} layout="responsive"></Image>
                    }
                </figure>
                <div className="buttons has-addons is-centered">
                        <button className="button is-size-5-fullhd is-size-6" onClick={()=>{
                            setTweetLoaded(false);
                        }}>
                            Go Back
                        </button>
                        ...
                        <button style={{display:canShare?"block":"none"}} className="button is-size-6 is-size-5-fullhd" onClick={()=>{
                            let f = new File([tweetImage.blob], tweetImage.name+".png", { type: tweetImage.blob.type });
                            navigator.share({
                                title: 'SHare Image',
